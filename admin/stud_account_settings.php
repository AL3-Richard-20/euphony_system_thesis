...
<html lang = "en">
	...
	<head>
		...
		<title>Euphony | Edit Student Account</title>
  	...
  	<body>
		...
		<?php
			...
			if(isset($_GET['userid'])){
				...
				$query_user_info = studInfo($the_user_id);
				...
				$email 			= escape($_POST['email']);
		  		...
		  		if(password_verify($oldpass, $user_password)){
			  		...
			  		if($update_user_account == 1){
						...
						$stud_fullname = "Edit ".$firstname. " ". $lastname. " account information";

						$query2 = "INSERT INTO activity_log (Date, Time, Detail, User_Id) ";
						$query2 .="VALUES (curdate(), curtime(), '$stud_fullname', '$user_id') ";
						$add_to_logs = mysqli_query($con, $query2);

						confirmQuery($add_to_logs);

						echo "<script>sweetAlert('success', 'Successfully Updated!', 'Student account was updated', 'stud_profile_settings.php?userid=$the_user_id');</script>";
					}	
			  	}

			  	else{

			  		$stud_fullname = "Edit ".$firstname. " ". $lastname. " account information attempt";

			  		$query = "INSERT INTO activity_log (Date, Time, Detail, User_Id) ";
					$query .="VALUES (curdate(), curtime(), '$stud_fullname', '$user_id') ";
					$add_to_logs = mysqli_query($con, $query);

					confirmQuery($add_to_logs);	

			  		echo "<script>sweetAlert('error', 'Unsuccessful', 'Old password is invalid', 'stud_profile_settings.php?userid=$the_user_id');</script>";	
  			...
  			<form method="POST" novalidate>
	  			...
	  			<div class="panel panel-default">
	  				...
	  				<div class="panel-header">
	  					<center><h3 class="cap">Edit Student Account</h3></center>
	  				</div>
	  				...
	  				<div class="panel-footer">
	  					...
	  					<center><button type="submit" class="btn btn-success btn-lg" id="send">Save</button>
					  	<button type="button" class="btn btn-default btn-lg" onclick="location.href='stud_profile_settings.php?userid=<?php echo $the_user_id; ?>';">Cancel</button></center>

        ...
        function Branch($con){
        	...
        	if(isset($_GET['salesid'])){
        		...
        		while($row = mysqli_fetch_assoc($query_trans)){
                    ...
                    $discount       = escape($row['Total_discount']);
                    $cash           = escape($row['Cash']);

        			$this->SetFont('Times','', 9);
                    $this->Cell(290,30, date('M d, Y', strtotime($trans_date)), 0, 0, 'C');
                    $this->Ln(5);
    				$this->Cell(170,30, number_format($total,2), 0, 0, 'C');
                    $this->Ln(5);
                    $this->Cell(170,30, $discount . " %", 0, 0, 'C');
    				$this->Ln(5);
    				$this->Cell(214,30, '', 0, 0, 'C');
    				$this->Ln(5);
    				$this->Cell(200,30, '', 0, 0, 'C');
    				$this->Ln(5);
    				$this->Cell(170,30, number_format($total,2), 0, 0, 'C');
    				$this->Ln(5);
    				$this->Cell(204,30, 'the sum of:', 0, 0, 'C');
    				$this->Ln(5);
        		...
        		if($query_trans == TRUE){
        			...
        			while($row2 = mysqli_fetch_assoc($query_all_items)){
                        ...
                        $order_quantity   = $row2['Quantity'];
               			...
               			$this->Ln(5);
    					$this->Cell(260,30, $order_quantity. " " .$order_prod_name. ",", 0, 0, 'C');
